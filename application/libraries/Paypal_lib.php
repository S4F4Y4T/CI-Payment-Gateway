...
class Paypal_lib
{
    ...
    function __construct($paypalConfig){
        $this->CI =& get_instance();
        $this->CI->load->helper('url');
        $this->CI->load->helper('form');
//        $this->CI->load->config('paypal');

        $sanbox = $paypalConfig['sandbox'];
        $this->paypal_url = ($sanbox == TRUE)?'https://www.sandbox.paypal.com/cgi-bin/webscr':'https://www.paypal.com/cgi-bin/webscr';

        $this->last_error = '';
        $this->ipn_response = '';

        $this->ipn_log_file = $paypalConfig['paypal_lib_ipn_log_file'];
        $this->ipn_log = $paypalConfig['paypal_lib_ipn_log'];

        $this->button_path = $paypalConfig['paypal_lib_button_path'];

        // populate $fields array with a few default values.
        // values can be overwritten by the calling script.
        $businessEmail = $paypalConfig['business'];
        $this->add_field('business',$businessEmail);
        $this->add_field('rm','2');    // Return method = POST
        $this->add_field('cmd','_xclick');

        $this->add_field('currency_code', $paypalConfig['paypal_lib_currency_code']);
